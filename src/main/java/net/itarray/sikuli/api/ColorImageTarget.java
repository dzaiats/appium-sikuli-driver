...
import net.itarray.sikuli.api.SearchByTextureAndColorAtOriginalResolution.ColorRegionMatch;
import net.itarray.sikuli.core.cv.ImagePreprocessor;
import net.itarray.sikuli.core.cv.VisionUtils;
import net.itarray.sikuli.core.search.TemplateMatcher.Result;
...
public class ColorImageTarget extends ImageTarget {
	...
	protected List<ScreenRegion> getUnorderedMatches(ScreenRegion screenRegion){
		...
		SearchByTextureAndColorAtOriginalResolution s = new
                SearchByTextureAndColorAtOriginalResolution(IplImage.create(screenImage.getWidth(), screenImage.getHeight(),1, 1),
				IplImage.create(targetImage.getWidth(), targetImage.getHeight(),1, 1));
		
		s.execute();		
		final List<Result> matches = Lists.newArrayList();
